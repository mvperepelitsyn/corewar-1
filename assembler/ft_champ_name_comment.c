...
int		ft_get_name_or_comment(t_asm_data *asm_data, char *line, int i, int k)
{
	...
	if (line[j++] == '"')
	{
		...
		if (!line[j])
		{
			MACHINE_WAIT_COMMENT ? (CHAMP_COMMENT = ft_strjoin(CHAMP_COMMENT, ft_strdup(&line[temp + 1]))) \
...
int		ft_add_text_in_quotes(t_asm_data *asm_data, char *line, int i)
{
	...
	{
		CHAMP_NAME = ft_strjoin(CHAMP_NAME, ft_strjoin("\n", ft_strsub(line, 0, i)));
		if (line[i] == '"')
		{
	...
	{
		CHAMP_COMMENT = ft_strjoin(CHAMP_COMMENT, ft_strjoin("\n", ft_strsub(line, 0, i)));
		if (line[i] == '"')
		{
			MACHINE_DOUBLE_QUOTES = 0;
