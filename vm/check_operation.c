...
void		calc_shift(t_carry *cr, t_cycle *cycle)
{
	...
	i = 0;
	cr->jump_len++;
	while (i < cmd_prms[cr->cmd_code - 1].prms_count)
	{
		if (cycle->descript[i] == 1)
			cr->jump_len++;
		else if (cycle->descript[i] == 2)
			cr->jump_len += (int)cmd_prms[cr->cmd_code - 1].dir_size;
		else
			cr->jump_len += 2;
		i++;
	}
...
static int	check_reg(t_vm *vm, t_carry *cr, t_cycle *cycle)
{
	...
	while (i < cmd_prms[cr->cmd_code - 1].prms_count)
	{
		...
		if (cycle->descript[i] == 1)
			...
			shift += (int)cmd_prms[cr->cmd_code - 1].dir_size;
		else
			shift += 2;
		i++;
	}
...
static int	check_params_type(t_carry *cr, t_cycle *cycle)
{
	...
	{
		if ((cycle->descript[i] == 1 && \
				!(cmd_prms[cr->cmd_code - 1].prm_types[i] & 1)) || \
				(cycle->descript[i] == 2 && \
				!(cmd_prms[cr->cmd_code - 1].prm_types[i] & 2)) || \
				(cycle->descript[i] == 3 && \
				!(cmd_prms[cr->cmd_code - 1].prm_types[i] & 4)))
			return (0);
		i++;
	}
	return (1);
...
static int	check_params(t_vm *vm, t_carry *cr, t_cycle *cycle)
{
	...
	if ((cmd_prms[cr->cmd_code - 1].prms_count < 3 && cycle->descript[2]) || \
			(cmd_prms[cr->cmd_code - 1].prms_count < 2 && cycle->descript[1]))
		return (0);
...
int			check_operation(t_vm *vm, t_carry *cr, t_cycle *cycle)
{
	if (!(cr->cmd_code >= 1 && cr->cmd_code <= 16))
		return (0);
	if (cmd_prms[cr->cmd_code - 1].descript)
	{
